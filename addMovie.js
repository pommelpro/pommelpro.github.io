...
function retrieveMovies () {
    ...
    query.find().then(function(results) {
        for (var i = 0; i < results.length; i++) {
            var created = results[i].createdAt
//            console.log(created);
//            var newMovie = {name: results[i].get("name"), medium: results[i].get("medium"), created: created.substring(0, created.length -15)};
            newArr[i] = newMovie; 
        }
...
function fillTable() {
    ...
    for (var j = 1; j < tableCells + 1; j++) {
        ...
        cell3.innerHTML = newArr[j-1].created;
        cell3.id = newArr[j-1].created;
